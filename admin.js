...
async function saveCompetition(){
  ...
  if(registration_deadline && Number.isNaN(new Date(registration_deadline).getTime())){msg('competitionMsg','報名截止時間格式不正確。');return false} if(checkin_points<0||!Number.isInteger(checkin_points)){msg('competitionMsg','簽到積分必須是 0 以上整數。');return false} if(checkin_start_at&&checkin_end_at&&new Date(checkin_start_at)>=new Date(checkin_end_at)){msg('competitionMsg','簽到開始時間必須早於結束時間。');return false}
  ...
  try{
    const payload={name,category,event_date,description,registration_deadline,registration_capacity,registration_approval,registration_fields,checkin_enabled,checkin_code,checkin_start_at,checkin_end_at,checkin_points};
