...
async function deleteGrowthReward(id){if(!confirm('確定要刪除這個獎勵嗎？此操作無法復原。'))return;try{const r=await fetch(SUPABASE_URL+'/rest/v1/growth_rewards?id=eq.'+encodeURIComponent(id),{method:'DELETE',headers:auth()});if(!r.ok){const d=await r.json().catch(()=>({}));throw new Error(d?.message||('HTTP '+r.status))}msg('rewardMsg','✅ 獎勵已刪除');await loadRewardManagers()}catch(e){msg('rewardMsg','❌ '+e.message)}}
let downloadFiles=[];
