...
const growthTaskTypeNames={profile_complete:'完成會員資料',first_registration:'第一次報名',first_result:'第一次參賽成績',registration_count:'報名次數',result_count:'成績次數'};
...
function clearGrowthTask(){['growthTaskId','growthTaskCode','growthTaskTitle','growthTaskDescription'].forEach(id=>{if($(id))$(id).value=''});if($('growthTaskType'))$('growthTaskType').value='profile_complete';if($('growthTaskRequirement'))$('growthTaskRequirement').value=1;if($('growthTaskReward'))$('growthTaskReward').value=10;if($('growthTaskActive'))$('growthTaskActive').checked=true;if($('growthTaskMsg'))$('growthTaskMsg').textContent=''}
function editGrowthTask(id){const x=growthTasks.find(v=>v.id===id);if(!x)return;$('growthTaskId').value=x.id;$('growthTaskCode').value=x.code;$('growthTaskTitle').value=x.title;$('growthTaskDescription').value=x.description||'';$('growthTaskType').value=x.task_type;$('growthTaskRequirement').value=x.requirement_count;$('growthTaskReward').value=x.reward_points;$('growthTaskActive').checked=!!x.active;window.scrollTo({top:$('growthTab').offsetTop-20,behavior:'smooth'})}
async function saveGrowthTask(){const id=$('growthTaskId').value.trim(),code=$('growthTaskCode').value.trim(),title=$('growthTaskTitle').value.trim(),description=$('growthTaskDescription').value.trim(),task_type=$('growthTaskType').value,requirement_count=Number($('growthTaskRequirement').value),reward_points=Number($('growthTaskReward').value),active=$('growthTaskActive').checked;if(!code||!title||!Number.isInteger(requirement_count)||requirement_count<1||!Number.isInteger(reward_points)||reward_points<0){msg('growthTaskMsg','請完整填寫任務資料。');return}try{const body={code,title,description,task_type,requirement_count,reward_points,active};const r=await fetch(SUPABASE_URL+'/rest/v1/growth_tasks'+(id?'?id=eq.'+encodeURIComponent(id):''),{method:id?'PATCH':'POST',headers:{...auth(),Prefer:'return=representation'},body:JSON.stringify(body)});const d=await r.json().catch(()=>null);if(!r.ok)throw new Error(d?.message||d?.hint||('HTTP '+r.status));msg('growthTaskMsg',id?'✅ 任務已更新':'✅ 任務已新增');clearGrowthTask();await loadGrowthManagers()}catch(e){msg('growthTaskMsg','❌ '+e.message)}}
