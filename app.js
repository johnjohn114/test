...
async function loadMyGrowth(){
  ...
  try{
    ...
    $('growthAchievements').innerHTML=ach.filter(x=>!x.hidden||am.has(x.id)).map(x=>{const a=am.get(x.id);const icon=x.icon||'🏆';const rarity={common:'普通',rare:'稀有',epic:'史詩',legendary:'傳說'}[x.rarity]||'普通';return '<div class="growthItem achievementItem"><div><b>'+esc(icon)+' '+esc(x.title)+'</b><p>'+esc(x.description||'')+'</p><small>'+esc(rarity)+' · 解鎖條件：'+esc(x.requirement_count)+' '+esc(x.achievement_type==='points'?'積分':x.achievement_type==='result_place'?'次冠軍':'次數')+(x.reward_points?' · 獎勵 💎 '+x.reward_points:'')+'</small></div><span>'+ (a?'🏆 已解鎖':'🔒 未解鎖')+'</span></div>'}).join('')||'<div class="empty">目前沒有成就。</div>';
    ...
    $('growthDashboardDetail').innerHTML='<div class="growthDashGrid"><div class="growthDashCard"><b>🎯 任務</b><strong>'+completedTasks+' / '+activeTasks+'</strong><span>目前完成的啟用任務</span></div><div class="growthDashCard"><b>🏆 成就</b><strong>'+unlockedAchievements+' / '+ach.length+'</strong><span>已解鎖成就</span></div><div class="growthDashCard"><b>📜 最近積分</b><strong>'+((hist[0]?.amount>0?'+':'')+(hist[0]?.amount||0))+' 💎</strong><span>'+esc(hist[0]?.reason||'尚無紀錄')+'</span></div><div class="growthDashCard"><b>⭐ 等級進度</b><strong>'+pctText+'</strong><span>'+nextText+'</span></div></div>';
